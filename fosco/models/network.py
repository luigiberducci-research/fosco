...
class RobustGate(TorchSymDiffModel):
    ...
    f(x) = m * (sigmoid(w x + b) - sigmoid(w x - b))

    see it here: https://www.desmos.com/calculator/2dgogzyqef

    m: multiplier which controls the output range, resulting in a function with range [0, m]
    w: weight, it controls the slope of the linear term, resulting in symmetric function which is more/less steep
    b: bias, it controls the center of the sigmoid, resulting in a symmetric fn which is more/less flat around 0

    Since we want it to be non-negative, m and b should be non-negative.
    ...
    def __init__(self, activation_type: str | ActivationType = "sigmoid"):
        ...
        self._sigmoid = partial(activation, select=activation_type)
        self._sigmoid_sym = partial(activation_sym, select=activation_type)
        self._sigmoid_der_sym = partial(activation_der_sym, select=activation_type)

        self.register_parameter(f"w", nn.Parameter(torch.randn(1, 1)))
        self.register_parameter(f"log_b", nn.Parameter(torch.randn(1, 1)))
        self.register_parameter(f"log_m", nn.Parameter(torch.randn(1, 1)))
    ...
    def forward_smt(
        ...
        term_pos = w * input_vars + b
        term_neg = w * input_vars - b

        z = m * (self._sigmoid_sym(p=term_pos) - self._sigmoid_sym(p=term_neg))
        z = z[0, 0]

        var_list = list(input_vars.flatten())
        return z, [], var_list
    ...
    def gradient_smt(
        ...
        """
        df(x)/dx = m * (sigmoid'(w x + b) - w * sigmoid'(w x - b)) =
                 = m * dsigmoid/dx(wx + b) * d(wx + b)/dx - m * dsigmoid/dx(wx - b) * d(wx - b)/dx =
                 = m * w * (dsigmoid/dx(wx + b) - dsigmoid/dx(wx - b))
        """
        input_vars = np.array(x).copy().reshape(-1, 1)
