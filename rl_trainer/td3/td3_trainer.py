...
class TD3Trainer(RLTrainer):
    def __init__(
        ...
        self.obs_space = envs.single_observation_space if hasattr(envs,
                                                                  "single_observation_space") else envs.observation_space
        self.act_space = envs.single_action_space if hasattr(envs, "single_action_space") else envs.action_space

        self.actor = DDPGActor(observation_space=self.obs_space,
                               action_space=self.act_space).to(device)
        self.qf1 = QNetwork(observation_space=self.obs_space,
                            action_space=self.act_space).to(device)
        self.qf2 = QNetwork(observation_space=self.obs_space,
                            action_space=self.act_space).to(device)
        self.qf1_target = QNetwork(observation_space=self.obs_space,
                                   action_space=self.act_space).to(device)
        self.qf2_target = QNetwork(observation_space=self.obs_space,
                                   action_space=self.act_space).to(device)
        self.target_actor = DDPGActor(observation_space=self.obs_space,
                                      action_space=self.act_space).to(device)
    ...
    def train(
            ...
            dones: torch.Tensor, next_obs: torch.Tensor,
            next_done: torch.Tensor = None, values: torch.Tensor = None, logprobs: torch.Tensor = None,
        ...
        args = self.args
        with torch.no_grad():
            clipped_noise = (torch.randn_like(actions, device=self.device) * args.policy_noise).clamp(
