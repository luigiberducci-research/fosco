...
def plot_rectangle(domain: Rectangle, fig: Figure, color: str=None,
                   dim_select: tuple[int,int]=None, label: str = "") -> Figure:
    """
    ...
    if color:
        ...
        single_color = None
    surface = Surface(z=Z, x=X, y=Y, colorscale=single_color, showscale=False, name=label)
    fig.add_trace(surface)

    return fig

def plot_sphere(domain: Sphere, fig: Figure, color: str, dim_select: tuple[int,int]=None, label: str = "") -> Figure:
    """
    ...
    single_color = [[0.0, color], [1.0, color]]
    surface = Surface(x=X, y=Y, z=Z, colorscale=single_color, showscale=False, name=label)
    fig.add_trace(surface)

    return fig

def plot_domain(domain: Set, fig: Figure, color: str, dim_select: tuple[int, int]=None, label: str = "") -> Figure:
    """
...
if __name__=="__main__":
    ...
    fig = plot_surface(func, (-5, 5), (-5, 5), levels=[0], label="surface", fig=fig, opacity=0.75)

    dim_select = (0, 1)
    fig = plot_domain(domain1, fig, color="red", dim_select=dim_select, label="domain1")
    fig = plot_domain(domain2, fig, color="blue", dim_select=dim_select, label="domain2")
