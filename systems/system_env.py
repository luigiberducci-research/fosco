...
class SystemEnv(gymnasium.Env):
    ...
    def _assert_input(self, system, domains):
        assert isinstance(system, ControlAffineControllableDynamicalModel), \
        ...
        assert isinstance(domains["init"], Set), f"init domain must be a Set, got {type(domains['init'])}"
        assert isinstance(domains["unsafe"], Set), f"unsafe domain must be a Set, got {type(domains['unsafe'])}"
        assert isinstance(domains["input"], Set), f"input domain must be a Set, got {type(domains['input'])}"

        assert isinstance(domains["input"], Rectangle), f"init domain must be a Rectangle, got {type(domains['init'])}"
        assert len(domains["input"].lower_bounds) == len(domains["input"].upper_bounds) == system.n_controls, \
